...
/* HVSS: determine the product classification
 * 0 = ionization 0+1+2
 * 1 = binsingle (0,1)+2
 * 2 = binsingle (0,2)+1
 * 3 = binsingle 0+(1,2)
 ...
 * 6 = merger 1:2 0
 * 7 = merger 0:1:2
 * -1 = there are two stars and at least one single, but none of them match the initial singles
 * -2 = binary
 * -3 = hierarchal triple
 * -4 = there are not 1, 2, or 3 stars
 */
int calc_type_f(fb_hier_t hier, fb_ret_t *retval, int id0, int id1, int id2) {

    int i, type_f;

    /* first check if three stars are present */
    if (hier.nstar==3) {
	/* now check if there are three objects */
	if (hier.nobj==3) {
	    type_f = 0;
	} else if (hier.nobj==2) {
        /* if there are two objects, iterate, identifying binaries */
            for (i=0; i<hier.nobj; i++) {
                if (hier.obj[i]->n==2) {
                    if ((hier.obj[i]->obj[0]->id==id0 && hier.obj[i]->obj[1]->id==id1) || (hier.obj[i]->obj[0]->id==id1 && hier.obj[i]->obj[1]->id==id0)) {
	    	    type_f = 1;
                    } else if ((hier.obj[i]->obj[0]->id==id0 && hier.obj[i]->obj[1]->id==id2) || (hier.obj[i]->obj[0]->id==id2 && hier.obj[i]->obj[1]->id==id0)) {
	    	    type_f = 2;
                    } else if ((hier.obj[i]->obj[0]->id==id1 && hier.obj[i]->obj[1]->id==id2) || (hier.obj[i]->obj[0]->id==id2 && hier.obj[i]->obj[1]->id==id1)) {
	    	    type_f = 3;
                    } 
                }
            }
	} else if (hier.nobj==1) {
	/* this should almost never happen, but if somehow there's a hierarchal triple, flag it */
	    type_f = -3;
	}
    } else if (hier.nstar==2) {
    /* now check if there are two stars, and if so identify single */
        for (i=0; i<hier.nobj; i++) {
	    if (hier.obj[i]->n==1) {
	        if (hier.obj[i]->id==id2) {
		    type_f = 4;
		} else if (hier.obj[i]->id==id1) {
		    type_f = 5;
		} else if (hier.obj[i]->id==id0) {
		    type_f = 6;
		} else {
		    type_f = -1;
		}
	    } else if (hier.obj[i]->n==2) {
		/* if there's only a binary */
	        type_f = -2;
	    }
	}
    } else if (hier.nstar==1) {
    /* check if there is one star */ 
        type_f = 7;
    } else {
    /* if somehow the number of stars is not 1, 2, or 3, flag it */
        type_f = -4;
    }
...
///* HVSS-OLD: print the product classification (bhbh+s, bhs+s, etc.) */
///* this is the old version, that doesn't match up with Carl's classification */
...
int main(int argc, char *argv[])
{
    ...
    int k0, k10, k11, id0, id10, id11;
    double a_fin, e_fin, Lbin[3], thetai;
	...
	for (j=0; j<3; j++) {
		Li[j] += Lint[j];
        	...
        	/* HVSS: save initial angular momentum of binary */
        	Lbin[j] = hier.obj[1]->Lmag * hier.obj[1]->Lhat[j];
	}

	/* HVSS: store ids of stars */
	id0 = hier.hier[hier.hi[1]+0].id;
	id10 = hier.hier[hier.hi[1]+1].id;
	id11 = hier.hier[hier.hi[1]+2].id;    
    ...
    calc_type_f(hier, &retval, id0, id10, id11);

    /* HVSS: print the things for python */
    fprintf(stdout, " %d", retval.type_f);
    fprintf(stdout, " %.15g", units.v/1.0e5); // /1.0e5 converts from cm/s to km/s
    fprintf(stdout, " %.15g %.15g", a_fin, e_fin);
    fprintf(stdout, " %.15g %.15g %.15g", Li[0], Li[1], Li[2]);
    fprintf(stdout, " %.15g %.15g %.15g", Lbin[0], Lbin[1], Lbin[2]);
    fprintf(stdout, " %.15g %.15g", Ei, retval.DeltaEfrac); //ADDED DURING REVAMP - COMMENT OUT TO WORK WITH OLD CODE
    if (hier.nobj == 3) {
        fprintf(stdout, " %.15g %d %.15g %d", hier.obj[0]->vfin, hier.obj[0]->k_type, hier.obj[0]->Rmin, hier.obj[0]->Rmin_j);
        fprintf(stdout, " %.15g %d %.15g %d", hier.obj[1]->vfin, hier.obj[1]->k_type, hier.obj[1]->Rmin, hier.obj[1]->Rmin_j);
        fprintf(stdout, " %.15g %d %.15g %d", hier.obj[2]->vfin, hier.obj[2]->k_type, hier.obj[2]->Rmin, hier.obj[2]->Rmin_j);
    } else if (hier.nobj == 2) {
        fprintf(stdout, " %.15g %d %.15g %d", hier.obj[0]->vfin, hier.obj[0]->k_type, hier.obj[0]->Rmin, hier.obj[0]->Rmin_j);
        fprintf(stdout, " %.15g %d %.15g %d", hier.obj[1]->vfin, hier.obj[1]->k_type, hier.obj[1]->Rmin, hier.obj[1]->Rmin_j);
        fprintf(stdout, " %.15g %d %.15g %d", 0., -1, 0., 0);
    } else if (hier.nobj == 1) {
        fprintf(stdout, " %.15g %d %.15g %d", hier.obj[0]->vfin, hier.obj[0]->k_type, hier.obj[0]->Rmin, hier.obj[0]->Rmin_j);
        fprintf(stdout, " %.15g %d %.15g %d", 0., -1, 0., 0);
        fprintf(stdout, " %.15g %d %.15g %d", 0., -1, 0., 0);
