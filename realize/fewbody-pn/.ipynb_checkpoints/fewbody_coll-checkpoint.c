...
int fb_is_collision(double r, double R1, double R2, double M1, double M2, double k1, double k2, double mass_units, double length_units, double bhns_tde_flag)
{
    ...
    if (r < collisions_multiple*(R1 + R2)) {
        fb_dprintf("Performed a fb collision. Merging stars: k1=%g, k2=%g, M1=%g, M2=%g, R1=%g, R2=%g, Beta=%g, impact param=%g, r_peri=%g \n", k1, k2, M1, M2,R1, R2, collisions_multiple, collisions_multiple*(R1 + R2), r);
        return(1);
    } else {
        return(0);
    }
...
int fb_collide(fb_hier_t *hier, double f_exp, fb_units_t units, gsl_rng *rng, double bh_reff, fb_input_t input)
{
	...
	while (cont) {
		...
		if (cont) {
			...
			fb_merge(&(hier->hier[hier->hi[1]+i]), &(hier->hier[hier->hi[1]+j]), hier->nstarinit, f_exp, units, rng,  bh_reff);
...
void fb_merge(fb_obj_t *obj1, fb_obj_t *obj2, int nstarinit, double f_exp, fb_units_t units, gsl_rng *rng, double bh_reff)
{
	...
	double a_at_merger, e_at_merger;
	fb_obj_t tmpobj;
	
	clight = FB_CONST_C / units.v;

    /* HVSS: collision matrix from Hurley 2002, table 2 */
    int coll_matrix[15][15] = {
	...
	if (obj1->k_type == 14 && obj2->k_type == 14){
		...
		fb_bh_merger(obj1->m, obj2->m, obj1->chi, obj2->chi, &mass_frac, &afinal, &v_perp, &v_para, rng);
    ...
    /* HVSS: update k_type with collision matrix */
    tmpobj.k_type = coll_matrix[obj1->k_type][obj2->k_type];
...
void fb_bh_merger(double m1, double m2, double a1, double a2, 
				  ...
				  double *v_perp, double *v_para, gsl_rng *rng)
{
