...
*/
	...
	hier.hier[hier.hi[1]+2].R = 2 * FB_REFF_BH * FB_CONST_G*m10/(FB_CONST_C*FB_CONST_C);
	hier.hier[hier.hi[1]+3].R = 2 * FB_REFF_BH * FB_CONST_G*m11/(FB_CONST_C*FB_CONST_C);
	...
	printf("L0=%.6g DeltaL/L0=%.6g DeltaL=%.6g\n", mu*b*vinf, fb_mod(L)/(mu*b*vinf)-1.0, fb_mod(L)-mu*b*vinf);
	printf("E0=%.6g DeltaE/E0=%.6g DeltaE=%.6g\n\n", Ei, E/Ei-1.0, E-Ei);

	/* trickle down the binary properties, then back up */
	for (j=0; j<2; j++) {
		printf("obj[%d]->a=%e\n", j, hier.obj[j]->a*units.l/FB_CONST_AU);
		printf("obj[%d]->e=%e\n", j, hier.obj[j]->e);
		printf("obj[%d]->m=%e\n", j, hier.obj[j]->m);
