...
typedef struct fb_obj{
	...
	double x[3]; /* position */
	double v[3]; /* velocity */
    double vfin; /* HVSS: speed at infinity post-encounter */
	int n; /* total number of stars in hierarchy */
	struct fb_obj *obj[2]; /* pointers to children */
	double a; /* semimajor axis */
	double e; /* eccentricity */
    double Lmag; /* HVSS: angular momentum magnitude */
	double Lhat[3]; /* angular momentum unit vector */
	...
	double last_pn_E;
	double last_pn_J;
    double Rmin; /* HVSS: Rmin for this object */
	int Rmin_j; /* HVSS: index of other object in Rmin encounter */
} fb_obj_t;
...
typedef struct{
    ...
    int type_f; /* HVSS: classification index of outcome */
} fb_ret_t;
...
int fb_collide(fb_hier_t *hier, double f_exp, fb_units_t units, gsl_rng *rng, double bh_reff, fb_input_t input);
void fb_merge(fb_obj_t *obj1, fb_obj_t *obj2, int nstarinit, double f_exp, fb_units_t units, gsl_rng *rng, double bh_reff);
void fb_bh_merger(double m1, double m2, double a1, double a2, double *mass_frac, double *afinal, double *v_para, double *v_perp, gsl_rng *rng);
...
/* The variadic macro syntax here is the old gcc standard, and compiles on
   Mac OSX with gcc. */
#define fb_dprintf(args...) if (fb_debug) fprintf(stdout, args)
...
extern int fb_debug;
double suppression, J_pn, E_pn, e_pn, v_kick, e_newt;

/* radiation rocket canonical kick speed */
#define FB_VKICK 120.0e5

/* effective BH radius (this times the schwarzschild radius); this has to be >~6.5, since otherwise 
