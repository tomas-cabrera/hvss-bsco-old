...
void fb_upsync(fb_obj_t *obj, double t)
{
	...
	for (i=0; i<3; i++) {
		...
		//printf("x0[%d]=%g v0[%d]=%g x1[%d]=%g v1[%d]=%g\n",i,obj->obj[0]->x[i],i,obj->obj[0]->v[i],i,obj->obj[1]->x[i],i,obj->obj[1]->v[i]);
	}
    ...
    /* HVSS: save magnitude of angular momentum */
    obj->Lmag = fb_mod(L);
