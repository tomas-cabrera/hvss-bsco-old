...
for gi, bse_k_group in enumerate(bse_k_groups):
    ...
    if plot_exists:
        ...
        #    handle.set_alpha(1)
        plt.savefig(hvss_utils.FIGS_PATH + "cmc_wbinned_vouthist_N_{}.pdf".format(bse_k_group.short_name))
        plt.close()
    else:
...
for gi, bse_k_group in enumerate(bse_k_groups):
    ...
    if plot_exists:
        ...
        # for handle in legend.legendHandles:
        #    handle.set_alpha(1)
        plt.savefig(hvss_utils.FIGS_PATH + "cmc_wbinned_vouthist_rv_{}.pdf".format(bse_k_group.short_name))
        plt.close()
    else:
        print("\t\tNo data found for {}; continuing...".format(bse_k_group.name))
        continue
...
for gi, bse_k_group in enumerate(bse_k_groups):
    ...
    if plot_exists:
        ...
        # for handle in legend.legendHandles:
        #    handle.set_alpha(1)
        plt.savefig(hvss_utils.FIGS_PATH + "cmc_wbinned_vouthist_age_{}.pdf".format(bse_k_group.short_name))
        plt.close()
    else:
        print("\t\tNo data found for {}; continuing...".format(bse_k_group.name))
        continue
